...
class sections:
    ...
    def decoupe_auto(self, stops=pd.DataFrame(), temps = 60, coef = 1.3):
        ...
        Parameters
        ----------
        stops : pandas.DataFrame(), optional
            Fichier stops du jeu GTFS.
        temps : int, optional
            Nombre d'heures en minutes, à multiplier par la taille de l'intervalle de temps. Par défaut : 60.
        coef : TYPE, optional
            Coefficient affine du nombre de trajets entre deux segments consécutifs. 
            Si le rapport du nombre de trajet est supérieur au coefficient, on créé un nouveau tronçon. Par défaut : 1.3.
    ...
    def _get_list_section_vfreq(self, stops, temps, coef):
        ...
        if stops.empty == False:        
                ...
                tmp.columns =  ['dep', "arr"]
                # conversion en string de toutes les colonnes à fusionner :
                tmp.dep = tmp.dep.astype(str)
                tmp.arr = tmp.arr.astype(str)
